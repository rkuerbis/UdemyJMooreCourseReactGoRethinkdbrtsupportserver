...
//			Changes(r.ChangesOpts{IncludeInitial: false}).
		...
		if err1 != nil {
			...
			fmt.Printf("subscribeChannelMessage(err1): %#v\n\n", err1)

			client.send <- Message{"error", "subscribeChannelMessage: " + err1.Error()}
			return
		}

        fmt.Printf("subscribeChannelMessage(cursor1): %#v\n\n", cursor1)



		cursor, err := r.Table("message").
//          getAll(1, {index: "channelId"}).orderBy("createdAt").
...
//			Changes(r.ChangesOpts{IncludeInitial: false}).
		...
		if err != nil {
			...
			fmt.Printf("subscribeChannelMessage: %#v\n\n", err)

			client.send <- Message{"error", "subscribeChannelMessage: " + err.Error()}
			return
		}
